# Wasm0 Docs
Wasm0 is the leading Web3 infrastructure company. It provides all the necessary blockchain infrastructure and services for companies to build new and bring their existing projects to Web3.

Wasm0 Docs contain information on Wasm0, from general product information to user guides, API references, tutorials, developer guides, and more.
The docs are targeted at existing and potential Wasm0 users and explain the product and how to use it, both as an end-user or developer.  
...
As an ever-developing company, we welcome you to contribute to Wasm0 Docs — via feedback raising an issue or a pull request adding/updating content.
...
If you're an Wasm0 user, use the GitHub web interface:
...
## Contribute to the project as an Wasm0 employee 
Now, if you're with us, you know to uphold our standards. 

If you are new to them, here are the guidelines:
1. Get acquainted with Wasm0 Docs gitflow.
2. Get acquainted with Wasm0 Docs deploying routine.
...
2. Follow the **branch naming format**. 
   ...
   * `hotfix` — on-the-spot updates
   ...
   Naming examples:
   * `feature/FOOBAR-1-create-wasm0-docs`
   * `bugfix/FOOBAR-2-fix-typos`
   * `hotfix/FOOBAR-3-update-wasm0-docs`
...
3. Follow the **pull request naming format**.
   ...
   Naming examples:
   * `feat: FOOBAR-555 add docs for Wasm0`
   * `fix(Wasm0 Docs): FOOBAR-666 fix typos`
   * `chore: FOOBAR-777 update smart contract addresses for Wasm0`
   
4. Make sure the **Assignee** is you and there are two **labels** — **documentation** and a section-specific label (e.g. **wasm0**). 


### Clone the repo

```shell
git clone git@github.com:Ankr-network/wasm-docs.git && cd wasm-docs
```
